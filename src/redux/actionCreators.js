...
export const fetchUsage = () => (dispatch) => {
  ...
  axios
    .get("https://py.rexopenwrt.repl.co/rawdata")
...
export const sliderLoaded = (stateJSON, sliderDate) => ({
  ...
  // Here comes the json file
  userUsagePayload: stateJSON.userUsage,
  routerUsagePayload: stateJSON.routerUsage,
...
export const fetchSlider = (sliderValue) => (dispatch) => {
  ...
  axios
    .post(GRAPHDATA_API, postEPOCH)
    .then((response) => response.data)
    .then((sliderSelectorState) =>
      dispatch(sliderLoaded(sliderSelectorState, sliderValue))
    )
    .catch((error) => console.log(error));
};
